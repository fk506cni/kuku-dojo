...
//
// 仕様:
// - localStorage キー `kuku_lang_preference` は SUPPORTED_LANGS のいずれか (Phase B 時点
//   ["auto","ja","en"]) を保存する。
...
test("loadLangPreference: SUPPORTED_LANGS 外の値 (旧バージョン / 手編集) は 'auto' に正規化", () => {
  // 第18回 C18-14 で Phase B 未実装言語に縮約済 — zh-CN/ko/vi/es/pt-BR は範囲外
  const cases = ["zh-CN", "zh-TW", "ko", "vi", "es", "pt-BR", "fr", "JA", "en-US", "", "xx", "123"];
...
test("saveLangPreference: SUPPORTED_LANGS 外は no-op で false を返し、保存しない (sticky check)", () => {
  const cases = ["zh-CN", "fr", "JA", "en-US", "", "xx", null, undefined, 123, true, ["ja"], { code: "ja" }];
  for (const v of cases) {
    const { Storage } = loadCore({
