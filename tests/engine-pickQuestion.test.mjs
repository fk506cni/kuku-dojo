...
test("pickQuestion: 同じ seed では同じ出題列が得られる（決定論）", () => {
...
test("pickQuestion: 重み 0 は抽選に出現しない（累積重みの単調性）", () => {
  const { Engine } = loadCore({ randomSeed: 5 });
  // 重み 0 は SPEC §2.3 下限 0.5 の規約外だが、累積加算で選ばれないことだけ確認。
  // 通常 updateWeight 経由では 0 は作られない。
  const weights = {};
