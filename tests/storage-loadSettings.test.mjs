...
test("loadSettings: lang サポート済み値 (SUPPORTED_LANGS 内) はそのまま保持", () => {
  // Phase C (v1.3.0): 初期 6 言語フル + auto に拡張。
  // Phase D で es/pt-BR を追加と同じ commit で SUPPORTED_LANGS と本テスト配列を 1 行ずつ拡張する。
  const supported = ["auto", "ja", "en", "zh-CN", "zh-TW", "ko", "vi"];
...
// Phase C (v1.3.0) 時点 — Phase D で追加予定の es/pt-BR は依然 SUPPORTED_LANGS から除外している。
// localStorage に保存されていても "auto" 正規化される。Phase D で言語実装と同じ commit で
// 本テストから対応値を移動 + 上の supported 配列に追加する運用。
test("loadSettings: Phase D 未実装言語 (es/pt-BR) は \"auto\" に正規化される", () => {
  const phaseD = ["es", "pt-BR"];
  for (const v of phaseD) {
    const { Storage } = withSettings("acct-pd-" + v, {
      soundEnabled: true,
      effectsEnabled: true,
      volume: 0.4,
      wrongWeightBoost: 1.0,
    ...
    assert.equal(Storage.loadSettings("acct-pd-" + v).lang, "auto");
  }
});
