...
test("loadSettings: lang サポート済み値 (SUPPORTED_LANGS 内) はそのまま保持", () => {
  // 第18回 C18-14: Phase B 時点では SUPPORTED_LANGS = ["auto", "ja", "en"] に縮約。
  // Phase C/D で言語追加と同じ commit で SUPPORTED_LANGS と本テスト配列を 1 行ずつ拡張する。
  const supported = ["auto", "ja", "en"];
...
// 第18回 C18-14: Phase B で未実装 6 言語 (zh-CN/zh-TW/ko/vi/es/pt-BR) は SUPPORTED_LANGS から
// 一時的に除外したため、localStorage に保存されていても "auto" 正規化される。Phase C/D で言語実装と
// 同じ commit で本テストから対応値を移動 + 上の supported 配列に追加する運用。
test("loadSettings: Phase B 未実装言語 (zh-CN/zh-TW/ko/vi/es/pt-BR) は \"auto\" に正規化される (C18-14)", () => {
  const phaseCD = ["zh-CN", "zh-TW", "ko", "vi", "es", "pt-BR"];
  for (const v of phaseCD) {
    const { Storage } = withSettings("acct-pcd-" + v, {
      soundEnabled: true,
      effectsEnabled: true,
      volume: 0.4,
      wrongWeightBoost: 1.0,
    ...
    assert.equal(Storage.loadSettings("acct-pcd-" + v).lang, "auto");
  }
});
