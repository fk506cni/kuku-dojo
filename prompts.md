...
> 参照: SPEC.md §8.9（第12回レビュー C12-08 / C12-17 / C12-18 / C12-19 / C12-20 / C12-21 反映版）
...
Phase A. I18n namespace 導入 (ja 単独、UI 変化なし)
  0. **Phase A 着手前の smoke test** (第13回 C13-05):
     - 開発版 `index.html` に以下の probe script を一時追加:
       ...
       ```
     - ブラウザで開き、DevTools Elements で `<head>`/`<body>` の <style> を検索し
       `.border` / `.flex` / `.grid` utility rule が JIT 生成されていないことを確認
     - 生成されていた場合は Play CDN が JSON script を走査している → 対策 A + tailwind.config.js blocklist 併用が必要 (SPEC §8.9.5 末尾を参照)
     - 確認後 probe script は削除し、本実装に入る

  1. MESSAGES オブジェクトを **`<script type="application/json">`** として分離配置 (SPEC §8.9.5 対策 A 準拠 / 第13回 C13-02 反映):
     - `index.html` の `<script type="text/babel">` より **前** に以下を配置:
       ```html
       <script type="application/json" id="kuku-messages">
       {"ja": { "app.title": "kuku-dojo", ... }}
       </script>
       ```
     - I18n namespace 初期化で `JSON.parse(document.getElementById("kuku-messages").textContent)` で読み込み:
       ```js
       const I18n = {
         messages: JSON.parse(document.getElementById("kuku-messages").textContent),
         current: null, // effectiveLang は Phase A-6 の detectLang で確定後に代入 (C13-15)
       };
       ```
     - **`const MESSAGES = { ja: {...} }` の JS 直書きは禁止** (Tailwind CLI 誤検出の原因 / C12-08)
  2. Util.t(key, params?) 関数を追加 (dot-separated key、パラメータ埋込は {name} 形式)
     - 数値は呼出側で `toFixed` / `toLocaleString` して string 化してから `{name}` に渡す (SPEC §8.9.4 追記 / C13-11)
  3. 既存のハードコード文言を MESSAGES.ja に抽出、JSX 側を Util.t(...) に置換
     - 残留ゼロを SPEC §8.9.5 の grep コマンドで確認
  4. Settings に lang: "auto" | "ja" | "en" | "zh-CN" | "zh-TW" | "ko" | "vi" | "es" | "pt-BR" を追加、DEFAULT は "auto"
  5. Storage.loadSettings で lang 未設定 / サポート外値は "auto" に正規化（C11-06 パターン）
  6. 初期化時に SPEC §8.9.3 の detectLang() でランタイム lang を確定し **`I18n.current`** に保持 (C13-15):
     - `const effectiveLang = settings.lang === "auto" ? detectLang() : settings.lang;`
     - `I18n.current = effectiveLang;` で React 外シングルトンに保存
     - SettingsModal で lang 変更時は (1) Storage.saveSettings (2) effectiveLang 再計算 + I18n.current 更新 (3) App 再描画 の順
     - navigator.languages の undefined / 空文字列ガード必須 (C12-19)
  7. SettingsModal「くわしい せってい」節に言語セレクトを追加 (§4.2.1 情報階層化方針)
  8. タイトル表示は常に "kuku-dojo"、副題のみ ja ロケール文言 (§8.9.2)
  9. **scripts/build-dist.mjs の Tailwind 対策 A 改修** (第13回 C13-14):
     - Tailwind CLI 呼出前に MESSAGES script を空 JSON に置換した一時 HTML を作成:
       ```js
       ...
       ```
     - smoke test 追加: `grep -cE '^\.(border|flex|grid|hidden|block|fixed|static|table)[\s{,]' dist/kuku-dojo.html`
       が想定外に増えていないこと (MESSAGES 由来の utility rule 流入ゼロ)
 10. 配布版ビルド成功 + Tailwind 誤検出 smoke test + 敵対的レビュー

Phase B. 英語 en 追加
  1. MESSAGES.en を追加、ja とキー一致をチェックする検証スクリプト (scripts/validate-i18n.mjs) を追加
     - **実行タイミング**（第13回 C13-13）: `scripts/build-dist.mjs` の先頭（index.html 読み込み前）で `execSync("node scripts/validate-i18n.mjs")` を呼び、失敗なら即 exit 1
     - SPEC §7.2.1.1 同期責任表に `validate-i18n.mjs` 行を追加（Phase B 実装時に反映）
     - 手動実行も可能（`node scripts/validate-i18n.mjs` だけで全言語のキー一致を検査）
  2. タイトル副題を SPEC §8.9.2 の表から採用 ("Multiplication Tables Practice")
  ...
  7. Phase B 完了時点で配布版サイズを実測、Phase D の可否を再評価 (C12-20)
  8. 敵対的レビュー

Phase C. 初期 6 言語フル (zh-CN / zh-TW / ko / vi)
  1. MESSAGES.zh-CN / zh-TW 追加 (繁体と簡体は別扱い / C12-17)
  2. zh-HK / zh-MO / zh-SG 等のフォールバックマッピング (SPEC §8.9.3)
  3. MESSAGES.ko / vi 追加 (副題は SPEC §8.9.2 の表から採用)
  4. validate-i18n.mjs を 6 言語で通す
  5. 敵対的レビュー

Phase D. 追加 2 言語 (需要次第)
  - es / pt-BR を需要を見て追加 (README Issues の反応ベース判断)
  - 配布版サイズを実測し、SPEC §7.4 の 1 MB zip 予算内であることを確認
```

### 自動判定ロジック (実装イメージ / SPEC §8.9.3 の参考実装を Step 12 用に再掲)
...
- [ ] 既存ハードコード文言を MESSAGES.ja に抽出 (残留ゼロを grep で確認 / 具体コマンドは「注意点」参照)
- [ ] MESSAGES ブロックの配置を Tailwind CLI 抽出から分離 (C12-08 対応: 別 `<script type="application/json">` / tailwind.config.js blocklist のいずれか)
- [ ] Settings.lang 追加 + マイグレーション (欠損補完 + サポート外値の正規化 / C12-21)
- [ ] detectLang() 実装 + SettingsModal 言語セレクト (`navigator.languages` の `undefined` / 空文字列要素ガード / C12-19)
- [ ] 配布版ビルド成功 + Tailwind smoke test OK + サイズ予算内 (SPEC §7.4 の新予算)
- [ ] 実機で日本語表示が現行と完全一致することを確認 (UI 退行ゼロ)
- [ ] 第14回敵対的レビュー (Phase A 完了後、docs/report14.md)

### 注意点

- Babel Standalone は `import` / `export` 非対応 → `messages/ja.json` の外部分離は不可、`const MESSAGES = {...}` 方式で index.html に埋込
- **Tailwind CLI 誤検出対策（C12-08）**: `--content index.html` は正規表現で文字列リテラルを走査する。MESSAGES オブジェクト内の `"border"` / `"flex"` / `"grid"` / `"hidden"` / `"block"` / `"fixed"` 等の英単語が utility class として誤抽出され、配布版 CSS に不要 rule が混入する。対策のどちらかを必ず採用:
  - (a) MESSAGES を別ブロック（例: `<script type="application/json" id="kuku-messages">`）に分離し、Tailwind content からは SPEC §7.2.1 smoke test で検証
  - (b) `tailwind.config.js` の `blocklist` に誤検出 utility 名を明示
- **ハードコード文言の残留検出（C12-09）**: Phase A 完了前に以下を実行し、出力が MESSAGES.ja 定義ブロック以外でゼロであることを確認:
  ```bash
  # 日本語文字 (ひらがな / カタカナ / 漢字) が Util.t() 呼出外に残っていないか
  rg -n '[\u3040-\u309F\u30A0-\u30FF\u4E00-\u9FFF]' index.html | rg -v 'Util\.t\('
  ```
- **タイトル翻訳方針（C12-18）**: タイトル本体は常に `kuku-dojo`（英字）、副題のみ各言語に翻訳する。`"くくどうじょう"` ひらがな表記は ja のみ。各言語の副題は Phase B / C で SPEC §8.9 の副題表を参照
- ひらがな主体 UI の tone を英語でも維持 (子供向け平易な単語)
- **README ロードマップの言語数（C12-17）**: README.md L213 を SPEC §8.9 の 8 言語表記に揃える（Phase A 着手前にドキュメント一括更新）
