...
- [ ] Settings JSDoc に `slowThresholdSec` 追加（SPEC §2.2 / §2.3 同期）
- [ ] `DEFAULT_SETTINGS` に `slowThresholdSec: 10` 追加
- [ ] `SLOW_THRESHOLD_PRESETS` 定数定義（index.html `WRONG_WEIGHT_BOOST_PRESETS` と同列）
- [ ] `Storage.loadSettings` マイグレーション: 欠損補完 + プリセット外正規化（C11-06 と同パターン）
- [ ] `Storage.loadSettings` の単体テスト: 旧 v1.0.x Settings → v1.1.0 Settings（slowThresholdSec が 10 で補完される）
- [ ] `Engine.isSlowAverage` 新設 + 単体テスト（データ無し / 境界値 / 正常値）
- [ ] `Engine.proficiencyLevel` / `Engine.updateWeight` に手を加えていないことを diff で確認（C12-01 / C12-04 解消の担保）
- [ ] `StatsHelpers.aggregate` 拡張 + 単体テスト（cold start 除外 3 問 / **単純平均**（C13-06）/ 最近 5 回配列 / per-cell 累積件数）
- [ ] `SettingsModal` に `slowThresholdSec` 4 段プリセット UI（`grid-cols-4 gap-2` / roving tabindex / ArrowKey / 第13回追加反映で 5 段→4 段化、旧 C13-03 解消）
- [ ] `SettingsModal` の情報階層化（案 C プログレッシブ開示 / 第13回 C13-04）: 基本 3 項目常時 + 「くわしい せってい」折りたたみで wrongWeightBoost + slowThresholdSec 配置
- [ ] Stats 画面「がんばった きろく」タブ実装（横棒グラフ / 「だいたい ○びょう」/ 到達可視化（mint 背景）/ バッジ 2 種 / サンプル不足メッセージ）
- [ ] 9×9 マップタブは時間要素を混ぜていないことを diff で確認（C12-02 解消の担保）
- [ ] タブ切替 a11y 規約に従う（第13回 C13-12）: role="tablist"/"tab" / ArrowKey / タブ切替で詳細カード閉じ / Esc は詳細カード閉じ → 画面遷移の優先順位
- [ ] Tailwind クラスは静的文字列のみ（C01-04 / `grid-cols-4` / 新規バッジ色クラス / 到達可視化用 `bg-emerald-50` `border-emerald-200` 等を正しく抽出）
- [ ] 中央値 / 標準偏差 / 分散 / 外れ値 等の統計用語が UI 文言にゼロ（C12-06 / 第13回 C13-10 grep 具体化）
      ```bash
      ...
      ```
- [ ] 配布版ビルド成功 + 外部 URL リーク検査 OK + サイズ予算内（SPEC §7.4 更新後の基準）
- [ ] SPEC.md §8.8 / §2.2 / §2.3 の同期更新
- [ ] **第16回**敵対的レビュー実施（docs/report16.md）— 第13〜15回は消化済み
- [ ] **リリースノート記載**（第13回 C13-09 / 追加反映）: GitHub Release v1.1.0 本文 + README.md ロードマップ節に「たつじん 5 秒はお子さん自身が選ぶ前提のチャレンジプリセット / 到達可視化のみでペナルティなし」と明記
