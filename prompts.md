...
- [ ] **前提: Step 11 + 第16回レビュー対応の変更が全て再現可能**（`npm test` → 49 件全緑、`npm run build` → 成功 359.8 KB、`docs/report16.md` の対応一覧で Major 4 / Minor 8 が「✅ 完了」状態）
- [ ] **前提: v1.0.1 タグが打たれている**（`git tag --list | grep v1.0.1` で確認）
- [ ] コミット 1 (F1 本体実装): F1 関連の modified / untracked を 1 コミットに束ね、commit message に `Refs: SPEC §8.8 / prompts.md Step 11 / docs/__archives/report16.md` を含める
- [ ] コミット 2 (version bump): `package.json` `"version": "1.1.0"` + `index.html` L5 ヘッダ同期（SPEC §7.5 刻印同期リスト）
- [ ] コミット 3 (CHANGELOG / README): `CHANGELOG.md` に `[1.1.0]` セクション追加、`README.md` の機能紹介 / ロードマップ節を更新
- [ ] `npm run build` 再実行で `dist/kuku-dojo.html` が 1.1.0 で再生成、`grep "Version:" dist/kuku-dojo.html` で `Version: 1.1.0 (配布版 / dist/kuku-dojo.html)` を確認
- [ ] 外部 URL リーク検査が再び OK（ビルドログで `外部 URL リーク検査: OK (CDN 参照ゼロ)` を確認）
- [ ] `git tag -a v1.1.0 -m "v1.1.0: 回答時間の可視化と じかんもくひょう"` → `git push origin main` → `git push origin v1.1.0`
- [ ] GitHub Release v1.1.0 本文作成（CHANGELOG.md [1.1.0] セクション + 冒頭の子供/保護者向け要約 + 「ペナルティなし」明記）
- [ ] GitHub Release v1.1.0 に `dist/kuku-dojo.html` (359.8 KB) をアセットとして添付
- [ ] `docs/__archives/` への退避: `docs/start16.md` / `docs/report16.md` を `docs/__archives/` に移動（CLAUDE.md §4 運用ルール）
- [ ] `prompts.md` Step 11.5 のチェックリストも完了反映（本 Step 自身の消化記録）
- [ ] NOTES.md に v1.1.0 リリース完了と次フェーズ（C11-23 実機検証 9 項目 / F2 多言語対応準備）を取消線 / 新規エントリで更新
