...
- 🌐 **多言語対応 (Languages: ja / en)** — ブラウザ言語を自動判定、せっていから手動切替も可能（Phase B / v1.2.0 時点で日英 2 言語）
...
- **v1.2.0**: 多言語対応（`navigator.language` による自動判定、せっていから手動切替）。Phase A (I18n 基盤) + Phase B (ja / en) は実装済、Phase C (zh-CN / zh-TW / ko / vi) と Phase D (es / pt-BR) は配布版サイズ予算と需要を見て順次追加予定（SPEC.md §8.9）
