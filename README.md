...
- 🌐 **多言語対応 (Languages: ja / en)** — ブラウザ言語を自動判定、せっていから手動切替も可能。最後に選んだ言語はログアウト後の Login 画面でも復元される（v1.2.0 / 日英 2 言語、Phase C で zh-CN / zh-TW / ko / vi、Phase D で es / pt-BR を追加予定）
...
- **v1.3.0**: 多言語対応 Phase C（zh-CN / zh-TW / ko / vi の 4 言語追加）。需要次第で Phase D（es / pt-BR）に進む。配布版サイズ予算（SPEC §7.4 の 1 MB zip）内で順次追加予定
