...
### 8.8 回答時間による苦手検出 (v1.1.0 予定)

**動機**: 正解したが回答に時間がかかっている問題は「瞬時に思い出せていない = 習熟が浅い」状態を示す。現状の適応アルゴリズムは「正誤」と「重み」のみで判定しており、この「時間苦手」を検出できない。

**前提**: `SessionResult.details[].responseTime` は既に ms 単位で記録済み（§2.2 QuestionDetail）。集計側で未使用なだけで、データは v0.x 時点から蓄積されている。

**設計方針**:

1. **cold start の除外**: セッション開始直後の数問（既定 3 問）はウォームアップとして `responseTime` 集計対象から除外。ユーザーがテンキー操作に慣れる時間や Web Audio API の初回 resume 等を「純粋な思考時間」から切り離すため。
2. **しきい値**: セッション内の `responseTime` の中央値 + 標準偏差で「遅い回答」を判定。絶対値（例: > 8 秒）の併用も検討
3. **proficiencyLevel 拡張**: `Engine.proficiencyLevel(weight, correctRate, attempts, avgResponseTime?)` に第 4 引数 `avgResponseTime` (ms) をオプショナル追加。未指定時は現行挙動（後方互換）
4. **重み更新への影響**: 「正解だが時間が遅い」場合に `updateWeight` で弱い重み加算（例: +0.5）を検討。ただし既存挙動を壊さないよう Settings で ON/OFF 可能にする（`responseTimeSensitivity: 0.0 | 0.5 | 1.0` 等）
5. **UI 反映**: Stats 画面 9×9 マップの色に time-based 要素を混ぜる or 新しい「じかんがかかった」ワースト 5 カードを追加

**注意点**:
- 7〜9 歳の子供はテンキー入力自体が遅い場合がある。しきい値は実データを見て調整、初版は保守的に
- AudioContext の初回 resume 遅延（iOS Safari）は responseTime に混じる。cold start 除外はこの緩和も兼ねる
- `elapsedTime` と `responseTime` の総和は必ずしも一致しない（マスコット celebrate 中の stall 等で誤差あり）
...
| `en` | 英語 | グローバル最大言語、タイトル例 "Multiplication Dojo" |
...
2. 言語タグを優先順で matching:
   - 完全一致（`zh-TW` → `zh-TW`）
   - 主要タグ一致（`en-US` → `en`）
   - 派生フォールバック（`zh-HK` → `zh-TW`、`pt-PT` → `pt-BR`）
3. `navigator.languages[]` を順に走査
4. 最終フォールバック: `ja`

**手動 override**: SettingsModal に言語セレクトを追加。`lang: "auto" | "ja" | "en" | ...` を Settings に持たせる。

**実装方針（Babel Standalone 制約下）**:
...
- `Util.t(key, locale?)` で現在言語の翻訳を取得、fallback chain は 現行言語 → `ja` → キー文字列そのまま
- Settings JSDoc に `lang` フィールド追加、DEFAULT_SETTINGS に `lang: "auto"` を追加

**翻訳時の注意**:

- タイトル「くくどうじょう」「kuku-dojo」はブランドとして保持し、副題で翻訳する運用を推奨（例: "kuku-dojo — Multiplication Tables Practice"）
- ひらがな主体 UI の tone に合わせ、英訳では子供向けの平易な単語を選ぶ（"Nice!" "Oops!" "Awesome!" 等）
- 数字表記はアラビア数字で全言語統一可能（実装負担が軽い）
- a11y 文言（`aria-label` / `role="radio"` の aria-label）も翻訳対象

**配布版サイズへの影響**:

- 1 言語あたり +3〜5 KB 推定（UI 文言 100〜200 キー × 平均 30 文字）
- 500 KB 予算の残 165 KB で 30〜50 言語相当。当面予算圧迫要因にならない

**段階実装の推奨順序**:

1. `I18n` namespace と `Util.t()` を ja 単独で導入（構造リファクタのみ、UI 変化なし）
2. `en` を追加、動作確認（Settings に言語セレクト追加）
3. `zh-CN` / `zh-TW` / `ko` / `vi` を順次追加
4. `es` / `pt-BR` は需要があれば追加

**互換性**:

- 既存アカウントの Settings に `lang` フィールドがない場合、`Storage.loadSettings` で `"auto"` に補完（前方互換マイグレーション）
- v1.0.x 以前のユーザーは初回起動時 `navigator.language` で自動判定され、従来の日本語 UI に復帰する（日本語ブラウザ利用者の場合）
