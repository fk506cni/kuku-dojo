...
| `kuku_lang_preference` | `string` | **device-level**（アカウント非依存）。直近にユーザーが Account 経由で確定した `Settings.lang` の生値 (`SUPPORTED_LANGS` のいずれか / Phase B 時点 `"auto" \| "ja" \| "en"`)。Login 画面 (Account 未確定時) で `applyEffectiveLang` の引数として参照し、ja 環境ブラウザ + en 設定 Account ユーザーの「ログアウト → Login が ja に戻る」UX を解消する (v1.2.0 Phase B / 第18回ユーザー判断 案 A) |
...
**`kuku_lang_preference` が device-level で運用される理由**（v1.2.0 Phase B / 第18回ユーザー判断 案 A）: Login 画面は Account 未確定なので Settings から lang を引けず、従来は `applyEffectiveLang("auto")` 経由で `navigator.language` に従っていた。これは ja 環境ブラウザで en 設定 Account を使う想定のとき「ログアウトすると Login 画面が ja に戻る」UX 退行を生む。device-level に直近の確定 lang を覚えておけば、Login も最後の言語で起動できる。複数の子供が違う言語を使う共用端末では「最後にログインした子の言語」が次回 Login に出る簡易動作で割り切る (Account ごとの正しい言語は Account 選択直後に切替わる)。値域は `SUPPORTED_LANGS` 完全一致のみ保持し、範囲外は読み出し時に `"auto"` に正規化する (Phase C/D で言語追加と同じ commit で値域も拡張)。書き込みタイミングは (1) `useEffect [currentAccountId]` で Account 確定時、(2) `updateSettings` で lang 変更時、の 2 経路。読み出しタイミングは (1) useState 初期化で `currentAccountId === null` のとき、(2) `useEffect [currentAccountId]` の null 経路 (ログアウト直後)。

### 2.2 型定義（JSDoc）

> **本節は Settings / Account / SessionResult / QuestionDetail / WeightMap の単一 source of truth**（第13回 C13-08 で明文化）。§8.8.2 / §8.9.9 等の機能追加章で示す JSDoc は「動機と値範囲の説明用」であり、型定義の正は本節に置く。v1.1.0 で `slowThresholdSec` を追加済み（F1 実装完了 / C14-29 / 第15回 C15-03）— `DEFAULT_SETTINGS` / `SLOW_THRESHOLD_PRESETS` / `Storage.loadSettings` マイグレーションは `index.html` の storage 節に実装済み（§8.8 参照）。v1.2.0 で `lang` を追加する時も同じパターンで**本節を先行更新**し、§8.9 側は「§2.2 を参照」表記に格下げする（C14-15: JSDoc 本体にプロセス指示を書かず、本 header に一元化）。
 ...
 * @property {"auto"|"ja"|"en"} [lang]
 *                                           - (v1.2.0 追加 / §8.9) UI 言語。`"auto"` はブラウザ検出 (detectLang)、
 *                                             それ以外は手動 override。Phase B 時点では実装済 2 言語 + auto に
 *                                             縮約 (第18回 C18-14)。SUPPORTED_LANGS = `["auto", "ja", "en"]` の 3 値、
 *                                             detectLang 戻り値は `"auto"` を含まない 2 値（§8.9.3）。Phase C で
 *                                             zh-CN/zh-TW/ko/vi、Phase D で es/pt-BR が追加されるごとに本 typedef と
 *                                             SUPPORTED_LANGS を 1 行ずつ拡張する運用 (MESSAGES 追加と同じ commit で同期)。
 *                                             `[?]` は raw 永続化値（v1.0.x / v1.1.0 からのマイグレーション対象 /
 *                                             第17回 C17-02）が optional であることを示す。`Storage.loadSettings`
 *                                             経由の戻り値は欠損時 `"auto"`、SUPPORTED_LANGS 外の値（Phase C/D 未実装
 *                                             言語含む）も `"auto"` に正規化されるため常に上記 3 値のいずれか（呼び
 *                                             出し側で防御は不要 / §8.9.9 マイグレーション契約）。
 */
```
...
| slowThresholdSec 初期（**v1.1.0 追加済み、§8.8**） | 10 (ふつう) |
...
| i18n キー整合検査（v1.2.0 Phase B 新設） | `scripts/validate-i18n.mjs`（ja を参照言語として全言語 MESSAGES のキー一致を検査。missing / extra どちらも exit 1） | `scripts/build-dist.mjs` 先頭で `execFileSync` により呼出し、失敗で build 停止 / 第13回 C13-13 |
| `<html lang>` と effective lang の同期（v1.2.0 Phase B / C17-09） | `index.html` 内の `applyEffectiveLang()` ヘルパー (I18n.current と `document.documentElement.lang` を同時更新) | useState 初期化 / アカウント切替 useEffect (`currentAccountId` truthy / null の 2 分岐) / updateSettings の計 4 箇所から呼出 (第18回 C18-08) |
...
### 8.8 回答時間の可視化と「じかんもくひょう」機能 (v1.1.0 実装済み)
...
5. **UI: Stats 画面に「がんばった きろく」タブを新設**（C12-16 解消 / 満足感を下げない設計）
   ...
   - セル単位の表示:
     ...
     - 直近と前回平均の差が -0.3 秒以上で、かつ **older サンプルが 2 件以上** (= `responseTimes` が 3 件以上) のときのみ「さいきんの へいきんより X びょう はやくなった！」を褒めバッジで表示（X は `Math.abs(diff).toFixed(1)` で小数点 1 桁固定 / 第13回 C13-11 / 第16回 C16-02 で文言を「このまえより」から複数平均を示唆する表現に変更、C16-07 で N=2 噪音を除外）
     - reached セル (薄ミント背景 + ★ prefix + `border-emerald-400`) では `fewSamples` 注記を**抑制する**（第16回 C16-04 で「到達しているのに『もっとやれ』」の UX 矛盾を解消）
     - 横棒グラフのサンプルラベルは `さいしん / 1かいまえ / 2かいまえ / 3かいまえ / 4かいまえ`（第16回 C16-01 で off-by-one 修正、`i + "かいまえ"` で N=「N 回前」を自然な日本語で示す）
     - 到達セルは色以外の冗長手がかりとして cell 本文先頭に **"★" 記号** を prepend し、色覚多様性下でも判別可能にする（第16回 C16-08 / WCAG 1.4.1）
     - 詳細カードに「さいきん {times.length}かいの へいきんだよ（ぜんぶで {cs.responseAttempts}かい とりくんだよ）」を補助表示し、`avgResponseMs`（直近最大 5 サンプル平均）と `responseAttempts`（lifetime 累積）の semantic を明示する（第16回 C16-11）
   - セッション単位のサマリ表示は **v1.1.0 MVP の範囲外**（第16回 C16-03 で仕様を狭める判断）:
     - 「きょうは だいたい ○びょうで こたえられたよ」および cold start 除外 3 問の「ウォームアップ」別表示は v1.1.x パッチまたは v1.2.0 以降で再検討する
     - 理由: per-cell 可視化で子供向けフィードバック (とうたつ / praise / slow) は十分成立しており、セッション単位サマリはハイブリッド運用の複雑化と引き換えに得られる情報量が限定的 (第16回レビュー判断)
     - 代替: time タブヘッダの注記「はじめの 3もんは ウォームアップとして かぞえないよ」で cold start 仕様の認知的可視化は最小限確保 (第16回 C16-12)
...
> **Settings 型の単一 source of truth は §2.2**（第13回 C13-08）。§2.2 の `@typedef Settings` に `slowThresholdSec` プロパティを **v1.1.0 で追加完了**（F1 実装済み / 第15回 C15-03）。本節は動機と値範囲の説明に留める（型定義の正は §2.2 を参照）。
