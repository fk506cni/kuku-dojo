...
> **Settings 型の単一 source of truth は §2.2**（第13回 C13-08）。v1.1.0 実装時に §2.2 の `@typedef Settings` に `slowThresholdSec` プロパティを追加する。本節は動機と値範囲の説明に留める。

追加フィールド:
- `slowThresholdSec`: number - (v1.1.0 追加) 遅い回答バッジのしきい値 (秒) / 15 (のんびり) / 10 (ふつう、既定) / 7 (きびきび) / 5 (はやめ) / 3 (たつじん)

`Storage.loadSettings` のマイグレーション契約:
- v1.0.x から v1.1.0 に上がった既存ユーザーの Settings に `slowThresholdSec` が無ければ、既定 10 で補完
- `slowThresholdSec` が 5 プリセット `{15, 10, 7, 5, 3}` の外の値（localStorage 手編集 / 壊れたデータ）なら 10 に正規化（C11-06 と同じパターン / C12-21 解消）
