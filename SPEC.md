...
> **本節は Settings / Account / SessionResult / QuestionDetail / WeightMap の単一 source of truth**（第13回 C13-08 で明文化）。§8.8.2 / §8.9.9 等の機能追加章で示す JSDoc は「動機と値範囲の説明用」であり、型定義の正は本節に置く。v1.1.0 予定の `slowThresholdSec` は本節 `@typedef Settings` に**仕様先行で記載済み**（C14-29 / 第15回 C15-03）— 実装側 `DEFAULT_SETTINGS` / `Storage.loadSettings` マイグレーションは F1 セッション（prompts.md Step 11 / §8.8）で追加する。v1.2.0 で `lang` を追加する時も同じパターンで**本節を先行更新**し、§8.9 側は「§2.2 を参照」表記に格下げする（C14-15: JSDoc 本体にプロセス指示を書かず、本 header に一元化）。
...
> **Settings 型の単一 source of truth は §2.2**（第13回 C13-08）。§2.2 の `@typedef Settings` には `slowThresholdSec` プロパティが**仕様先行で記載済み**（実装は F1 セッションで追加予定 / 第15回 C15-03）。本節は動機と値範囲の説明に留める（型定義の正は §2.2 を参照）。
